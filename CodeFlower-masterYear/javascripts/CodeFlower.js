var CodeFlower = function(selector, w, h) {
  ...
  this.force = d3.layout.force()
    ...
    .charge(function(d) { return d._children ? -d.size / 10000 : -40; })
...
CodeFlower.prototype.update = function(json) {
  ...
  this.node.transition()
    .attr("r", function(d) { return d.children ? 3.5 : Math.pow(d.size/20, 2/5) || 1; });

  // Enter any new nodes
  this.node.enter().append('svg:circle')
    .attr("class", "node")
    .classed('directory', function(d) { return (d._children || d.children) ? 1 : 0; })
    .attr("r", function(d) { return d.children ? 3.5 : Math.pow(d.size/20, 2/5) })
...
CodeFlower.prototype.mouseover = function(d) {
  ...
  d3.select("#tooltip3").style("visibility", "visible").text("Heart Rate: Level"+ d.hr );
};
